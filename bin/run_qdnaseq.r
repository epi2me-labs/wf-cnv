...
#p <- add_argument(p, "--reference", help="QDNAseq GC/mappability bins reference. Defaults to qdnaseq_hg38", default="QDNAseq.hg38")
...
#Plot raw copy numbers
highlightFilters(readCounts, logTransform=FALSE, residual=TRUE, blacklist=TRUE)

autosomalReadCountsFiltered <- applyFilters(readCounts, residual=TRUE, blacklist=TRUE, chromosomes=c('Y','MT'))
...
#Export bins
bedout <- paste(argv$out_prefix, "fembins.bed", sep="_")
exportBins(copyNumbersSmooth, bedout, format="bed") #ADD VARIABLE NAME
...
#write outputs
vcfout <- paste(argv$out_prefix, "femcalls.vcf", sep="_")
exportBins(copyNumbersCalled, file=vcfout, format="vcf", type="calls")
segout <- paste(argv$out_prefix, "femsegs.bed", sep="_")
exportBins(copyNumbersCalled, file=segout, format="bed", type="segments")
